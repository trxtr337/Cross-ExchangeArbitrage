        ...
        async with session.get(ticker_url) as ticker_response:
            if ticker_response.status == 200:
                ...
                for ticker in info_all_futures_tickers:
                    ...
                    lastPrice = float(ticker['lastPrice'])
                    fundingRate = 100 * float(ticker['fundingRate'])
                    # symbols_dict[symbol] = [lastPrice, fundingRate]
                    symbols_dict[symbol] = {
                        'lastPrice': lastPrice,
                        'fundingRate': fundingRate,
                    }
...
async def runMexc():
    future_info = await create_futures_info_dict_bybit()
    future_info_upd = await second_upd(future_info)
    return future_info_upd
